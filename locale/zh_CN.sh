...
_NC_STRINGS[core_stopped]="%s 已停止"
_NC_STRINGS[status_core]="核心:      %s"
_NC_STRINGS[status_status]="状态:      %s"
_NC_STRINGS[status_user]="用户:      %s"
_NC_STRINGS[status_profile]="订阅:      %s"
_NC_STRINGS[status_log_level]="日志级别:  %s"
_NC_STRINGS[status_bind]="监听地址:  %s"
_NC_STRINGS[status_mix_port]="混合端口:  %s"
_NC_STRINGS[status_tun_on]="虚拟网卡:  开启（协议栈：%s，接口：%s，MTU：%s）"
_NC_STRINGS[status_tun_off]="虚拟网卡:  关闭"
_NC_STRINGS[status_rules]="规则:      %s 条自定义"
_NC_STRINGS[status_geodata]="Geodata:   %s 个文件"
_NC_STRINGS[core_started_tun]="%s 已启动（虚拟网卡：%s）"
